...
Phonetiq/
  api/                        # Cloudflare Worker (Hono)
    ...
    scripts/
      run-eval.ts             # Dialect-tagged eval harness (+ schema-versioned JSON, run metadata, strict guardrails, family rollups)
      run-eval-experiment.ts  # Frame eval variant (+ schema-versioned JSON, run metadata, strict guardrails, family rollups)
