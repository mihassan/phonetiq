...
- Tests sit next to implementation as `*.test.ts(x)`; Vitest runs in both `web/` (158 tests) and `api/` (67 tests).
...
- Frame-sentence recognition is the production path; do not remove `matchFrameSentence` or `extractFrameWord` from `recognize.ts`.
...
# api
cd api && npm run dev
cd api && npm run dev:frame      # frame-sentence profile (port 8791)
cd api && npm run typecheck
cd api && npm test
cd api && npm run eval           # frame-sentence recognition eval
cd api && npm run eval:fast:frame # frame-sentence eval (no rate-limit delay)
...
- Eval ports: baseline=8787, frame=8791.
- Alternate recognition experiment flags were removed in favor of a single frame-sentence path.
