...
describe('DialectFilter', () => {
  it('renders dialect options and toggles aria-pressed', () => {
    render(<DialectFilter selected="us_only" onSelect={vi.fn()} />);

    expect(screen.queryByRole('button', { name: /common/i })).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: /us/i })).toHaveAttribute('aria-pressed', 'true');
  ...
  it('calls onSelect with chosen dialect', () => {
    ...
    render(<DialectFilter selected="us_only" onSelect={onSelect} />);
