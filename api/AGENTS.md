...
| Baseline eval harness | `scripts/run-eval.ts` | Dialect-tagged corpus with expanded pilot contrast families, per-dialect + contrast-family summaries, strict guardrails, and full/summary JSON output; `npm run eval` / `eval:fast` / `eval:json` / `eval:summary:json` / `eval:guard` |
| Frame eval harness | `scripts/run-eval-experiment.ts` | Same expanded dialect-tagged corpus on `dev:frame`, including contrast-family summaries, strict guardrails, and full/summary JSON output; `npm run eval:frame` / `eval:frame:json` / `eval:frame:summary:json` / `eval:frame:guard` |
